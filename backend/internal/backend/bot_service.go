	...
	decls := []*genai.FunctionDeclaration{
		{
			...
			Parameters: &genai.Schema{
				...
				Properties: map[string]*genai.Schema{
					...
					"metric": {
						Type:        genai.TypeString,
						Enum:        []string{"distance", "steps", "swim", "gym", "sessions", "activeDays", "longest", "pace"},
						Description: "Tiêu chí: distance = TỔNG km (chạy + đi bộ, khớp 'Tổng km' trong app), steps = tổng số bước chân, swim = tổng km BƠI, gym = tổng PHÚT tập gym/workout, sessions = số buổi chạy, activeDays = số ngày có chạy, longest = buổi chạy dài nhất, pace = tốc độ chạy tốt nhất.",
					},
